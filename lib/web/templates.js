...
//     @TH '.xxxheader', => delta_hex
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.harfbuzz = function(ctx) {
    var height, style, text, width;
    // debug '^33442^', ctx.query
    // url       = COMMON.get_url '/v2/slug', { fontnick, text, }
    ...
    return this.render(() => {
      this.layout();
      this.TITLE("SVG Typesetting with Harfbuzz and OpentypeJS");
      this.STYLE("body { background-color: #cf68; padding: 10mm; }");
      this.H1(() => {
        return "SVG Typesetting with Harfbuzz and OpentypeJS";
      });
      this.P(() => {
        this.TEXT(`SVG with glyph outlines provided by opentype.js, typesetting metrics by harfbuzz. Outlines
are implemented as SVG symbols accessible via external resources, which is why we must`);
        return this.UL(() => {
          this.LI(() => {
            ...
            this.CODE(() => {
              "<object>";
              return this.TEXT(" tags instead of ");
            });
            this.CODE("<img>");
            return this.TEXT(" tags (images are not allowed to reference external resources);");
          });
          return this.LI(() => {
            return this.TEXT(`use a web server and not open the current file directly in the browser, as it would then
not resolve the references in the loaded SVG.`);
      ...
      this.DIV(() => {
        this.DIV(() => {
          return this.CODE('', "/v2/font?fid=f123");
        });
        this.OBJECT({
          type: 'image/svg+xml',
          data: '/v2/font?fid=f123',
          style: 'outline:1px dotted red;width:250mm'
        }, () => {
          return this.DIV({
            style: 'outline:1px dotted blue;width:250mm'
          }, "(Fallback for SVG here)");
        });
        this.DIV(() => {
          return this.CODE('', "/v2/harfbuzz-opentypejs-slug?fid=f123&text=AEIÖU");
        });
        return this.OBJECT({
          type: 'image/svg+xml',
          data: '/v2/harfbuzz-opentypejs-slug?fid=f123&text=AEIÖU',
          style: 'outline:1px dotted red;width:250mm'
        }, () => {
          return this.DIV({
            style: 'outline:1px dotted blue;width:250mm'
      ...
      return this.DIV({
        style: "width:10mm;height:10mm;background-color:red;"
      });
    });
  };
