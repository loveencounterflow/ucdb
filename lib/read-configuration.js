(function() {
  ...
  (() => {    //-----------------------------------------------------------------------------------------------------------
    // ... precompute
    return this._cidrange_from_rsg = function(rsg) {
      return {
        first_cid: 111,
        last_cid: 111
      };
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._cidrange_from_text_with_rsgs = function(range_txt) {
    if (range_txt.startsWith('rsg:')) {
      return this._cidrange_from_rsg(range_txt.replace(/^rsg:/, ''));
    }
    return this._cidrange_from_text_without_rsgs(range_txt);
  };

  //-----------------------------------------------------------------------------------------------------------
  this._cidrange_from_text_without_rsgs = function(range_txt) {
    var R;
  ...
  this.compile_configurations = function(me) {
    var first_cid, last_cid, linenr, ref, ref1, row;
    me.db.prepare_configuration_tables();
    ref = me.dbr.read_configuration_rsgs_and_blocks();
    //.........................................................................................................
    for (row of ref) {
      ({linenr} = row);
      ({first_cid, last_cid} = this._cidrange_from_text_without_rsgs(row.range_txt));
    ...
    for (row of ref1) {
      ({linenr} = row);
      ({first_cid, last_cid} = this._cidrange_from_text_with_rsgs(row.range_txt));
