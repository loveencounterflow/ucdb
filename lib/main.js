(function() {
  'use strict';
  var $, $async, $show, $watch, CND, FONTMIRROR, FS, FSP, MAIN, MIRAGE, MKNCR, Multimix, PATH, PD, SVGTTF, UCDB, Ucdb, _drop_extension, _glob, alert, assign, badge, cast, cwd_abspath, cwd_relpath, debug, declare, echo, glob, help, here_abspath, info, isa, jr, last_of, log, mkts_glyph_styles, mkts_options, project_abspath, rpr, runmode, size_of, type_of, urge, validate, walk_cids_in_cid_range, warn, whisper,
    indexOf = [].indexOf;
  ...
  // MAIN TABLE
  //-----------------------------------------------------------------------------------------------------------
  this._XXX_get_all_glyphs_as_list_from_cfg_glyphsets = function(me) {
    /* TAINT to be replaced by a configuration that maps from runmodes to sets of glyphs and fonts */
    return Array.from((this._XXX_get_all_glyphstrings_from_cfg_glyphsets(me)).join(''));
  };

  this._XXX_get_all_glyphstrings_from_cfg_glyphsets = function(me) {
    var row;
    return (function() {
      var ref, results;
      ref = me.db.$.query("select setname, glyphs from cfg_glyphsets;");
      results = [];
      for (row of ref) {
        /* TAINT to be replaced by a configuration that maps from runmodes to sets of glyphs and fonts */
        results.push(row.glyphs);
      }
      return results;
    })();
  };
  ...
  this.populate_table_main = function(me) {
    return new Promise(async(resolve, reject) => {
      var cid, cid_range, cid_ranges, cids, csg, data, description, fncr, glyph, i, iclabel, is_u9cjkidg, last_idx, len, line_count, ncr, preamble, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, ref8, row, rsg, sfncr, sql, style, tags, xncr;
      preamble = [];
      data = [];
      line_count = 0;
      ...
      cid_ranges = this._XXX_get_all_glyphstrings_from_cfg_glyphsets(me);
//.........................................................................................................
/* TAINT see remark in method */      for (i = 0, len = cid_ranges.length; i < len; i++) {
  ...
  this.populate_table_outlines = function(me) {
    ...
    // XXX_includes      = 'kai'
    // XXX_includes      = """sunexta sourcehansansboldone sourcehansansboldsix sourcehansansboldthree"""
    XXX_includes = "babelstonehan";
    // XXX_includes      = """sunexta thtshynptwo thtshynpone fandolheibold
    //   sourcehansansboldone
    //   sourcehansansboldtwo
    //   sourcehansansboldthree
    //   sourcehansansboldfour
    //   sourcehansansboldfive
    //   sourcehansansboldsix
    //   sourcehansansboldseven
    //   sourcehansansboldeight
    //   sourcehanserifheavyjapan
    ...
    this._XXXX_add_cached_outlines(me, known_hashes, glyphrows);
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._XXXX_add_cached_outlines = function(me, known_hashes, glyphrows) {
    /* TAINT cache iclabels from DB */
    /* NOTE this method will replace `_insert_into_table_outlines()` */
    var advance, batch_size, cache_path, cid_hex, content, content_data, d, duplicate_count, excerpt, fontnick, glyph, hash, iclabel, line_count, outlines_data, pathdata, progress_count, ref, relpath, row;
    outlines_data = [];
    content_data = [];
    line_count = 0;
    duplicate_count = 0;
    ...
    fontnick = null;
    ref = FONTMIRROR.walk_cached_outlines(null, cache_path);
    for (d of ref) {
      switch (d.key) {
        case '^first':
          help('^2328^', jr(d));
        ...
        case '^outline':
          ...
          info('^56798^', (jr(d)).slice(0, 81));
          ({cid_hex, glyph, advance, pathdata} = d);
          if ((me._outline_count++ % progress_count) === 0) {
            whisper('^ucdb@1016^', me._outline_count - 1);
          }
  ...
  // #-----------------------------------------------------------------------------------------------------------
  // @XXX_read_cache = ( settings = null ) -> new Promise ( resolve, reject ) =>
  //   # pattern   = ///^0x(<?cid_hex>[0-9a-f]+),(?<glyph>.),(?<outline_json>.*)$///u
  //   # cachepath = project_abspath 'cache/b611fe09aef46f975'
  //   #   lines = ( ( require 'fs' ).readFileSync cachepath, { encoding: 'utf-8', } ).split '\n'
  //   #   for line in lines
  //   #     continue unless ( match = line.match pattern )?
  //   #     { cid_hex, glyph, outline_json, } = match
  //   #     outline = JSON.parse outline_json
