(function() {
  'use strict';
  var $, $async, $show, $watch, CND, FS, FSP, MKNCR, PATH, PD, _drop_extension, _glob, alert, assign, badge, cid_ranges, cid_ranges_by_runmode, cwd_abspath, cwd_relpath, debug, declare, echo, glob, help, here_abspath, info, isa, jr, last_of, log, mkts_fontfiles, mkts_glyph_styles, mkts_options, project_abspath, rpr, runmode, size_of, type_of, urge, validate, warn, whisper,
    indexOf = [].indexOf;
  ...
  cid_ranges_by_runmode = {
    ...
    debug_small: [[0x03002, 0x03002], [0x04df0, 0x04e02]],
    production: [
      [0x00001,
      0x000ff],
      [0x00100,
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.populate_fontnick_table = function(me) {
  ...
  //-----------------------------------------------------------------------------------------------------------
  this.populate_main_table = function(me) {
    return new Promise(async(resolve, reject) => {
...
//.........................................................................................................
      for (i = 0, len = cid_ranges.length; i < len; i++) {
        ...
        for (cid of ref) {
          if ((++line_count % 10000) === 0) {
            whisper('^77763^', CND.format_number(line_count));
          }
  ...
  this.create = function(settings = null) {
    return new Promise(async(resolve, reject) => {
      ...
      await this.populate_fontnick_table(me);
      await this.populate_main_table(me);
  ...
  this.write_ucdb = async function(settings = null) {
    var count, dts, error, ref, row, t0, t1, ucdb;
    t0 = Date.now();
    try {
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._find_fontnick_ranges = function(me = null) {
    var ref, results, row;
    if (me == null) {
      me = this.new_ucdb();
    }
    me.db.create_view_main_with_deltas_etc();
    ref = me.db.fontnick_boundaries();
    // for row from me.db.main_with_deltas()
    //   info '^77763^', row.iclabel, row.fontnick, row.rear_delta_cid, row.fore_delta_cid
    results = [];
    for (row of ref) {
      // help '^77456^', row
      results.push(help('^77763^', CND.yellow(row.fontnick.padEnd(30)), CND.blue(row.first_iclabel), CND.blue(row.last_iclabel), CND.grey(row.first_cid), CND.grey(row.last_cid)));
    }
    return results;
  };
