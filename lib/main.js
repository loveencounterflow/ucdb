(function() {
  'use strict';
  var $, $async, $show, $watch, CND, FONTMIRROR, FS, FSP, MAIN, MIRAGE, MKNCR, Multimix, PATH, PD, SVGTTF, UCDB, Ucdb, _drop_extension, _glob, alert, assign, badge, cache, cast, cwd_abspath, cwd_relpath, debug, declare, echo, glob, help, here_abspath, info, isa, jr, last_of, log, mkts_glyph_styles, mkts_options, project_abspath, rpr, runmode, size_of, type_of, urge, validate, walk_cids_in_cid_range, warn, whisper,
    indexOf = [].indexOf;
  ...
  this.populate_table_main = function(me) {
    return new Promise(async(resolve, reject) => {
      var cid, cid_range, cid_ranges, cids, csg, data, description, fncr, glyph, i, iclabel, is_u9cjkidg, last_idx, len, line_count, ncr, preamble, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, ref8, row, rsg, seen_cids/* TAINT see remark in method */, sfncr, sql, style, tags, xncr;
      preamble = [];
      data = [];
      line_count = 0;
...
//.........................................................................................................
  ...
  //-----------------------------------------------------------------------------------------------------------
  this._get_iclabels_by_glyphs = function(me) {
    var R, ref, row;
    if ((R = cache.iclabels_by_glyphs) != null) {
      return R;
    }
    R = {};
    ref = me.db.read_iclabels_and_glyphs();
    for (row of ref) {
  ...
  this._XXXX_add_cached_outlines = function(me, known_hashes, glyphrows) {
    ...
    var advance, batch_size, cache_path, cid_hex, content, content_data, d, duplicate_count, excerpt, fontnick, glyph, hash, iclabel, iclabels_by_glyphs, line_count, outlines_data, pathdata, progress_count, ref, relpath, row;
    iclabels_by_glyphs = this._get_iclabels_by_glyphs(me);
    outlines_data = [];
    content_data = [];
    line_count = 0;
    duplicate_count = 0;
    ...
    for (d of ref) {
      switch (d.key) {
        ...
        case '^outline':
          ...
          hash = MIRAGE.sha1sum_from_text(content);
          //...................................................................................................
          if ((iclabel = iclabels_by_glyphs[glyph]) == null) {
  ...
  this.create = function(settings = null) {
    return new Promise(async(resolve, reject) => {
      ...
      urge('ucdb/create@1/6');
      await this.read_configuration(me);
      urge('ucdb/create@2/6');
      await this.compile_glyphsets_configuration(me);
      urge('ucdb/create@3/6');
      await this.populate_table_fontnicks(me);
      urge('ucdb/create@4/6');
      await this.populate_table_main(me);
      urge('ucdb/create@5/6');
      await this.compile_configurations(me);
      urge('ucdb/create@6/6');
