(function() {
  'use strict';
  var $, $async, $echo, $show, $time_request, $watch, CND, FS, FSP, HTTP, Koa, O, PATH, PD, TIMER, _drop_extension, _glob, alert, assign, badge, cast, cid_from_cid_txt, cwd_abspath, cwd_relpath, debug, declare, echo, glob, help, here_abspath, info, isa, jr, last_of, log, project_abspath, root_router, rpr, size_of, svg_from_cid, type_of, urge, validate, walk_cids_in_cid_range, warn, whisper;
  ...
  (() => {
    ...
    return O.ucdb = (require('..')).new_ucdb({
      db_path: O.db_path
    });
  })();
  ...
  this.serve = function() {
    ...
    root_router.get('cats', '/cats', this.$cats());
    ...
    app.use($time_request()).use($echo()).use(root_router.routes()).use(root_router.allowedMethods());
    return app;
  };
